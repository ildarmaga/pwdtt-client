...
func TestRequestRawConfigOK(t *testing.T) {
	...
	go func() {
		...
		if strings.Contains(string(buf[:n]), "CHUNK1") {
			t.Errorf("default RAWCONF must not request CHUNK1: %q", string(buf[:n]))
		}
...
func TestRequestRawConfigAcceptsUnsolicitedChunk1Cap(t *testing.T) {
	c1, c2 := net.Pipe()
	defer c1.Close()
	defer c2.Close()
	go func() {
		...
		n, _ := c2.Read(buf)
		if strings.Contains(string(buf[:n]), "CHUNK1") {
			t.Errorf("default handshake requested CHUNK1: %q", string(buf[:n]))
		}
		_, _ = c2.Write([]byte("IP = 10.70.66.3\nDNS = 8.8.8.8\nMTU = 1280\nCAPS = CHUNK1\n"))
	}()
	resp, err := RequestRawConfig(c1, "dev1", "secret", 1280)
