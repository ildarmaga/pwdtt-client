...
### Changelog (2026-06-13, ч.2)

- **Брендинг** — приложение переименовано `PWDTT` → **WDTT** (заголовок окна, трей, автозапуск, User-Agent).
- **Окно** — фиксированный размер **650×700**, изменение размера запрещено (`DisableResize`).
- **MTU** — значение по умолчанию **1280** (пользователь может изменить в настройках).
- **Windows** — приложение всегда запускается **от имени администратора** (`requireAdministrator` в манифесте).
- **Стабильность туннеля** — backoff при переподключении, лимит параллельных TURN Allocate (4) и DTLS handshake (6), таймаут handshake 45 c.
- **Сервер** — relay eviction поднят до 3 мин, first-read дедлайн ≥45 c.
- **Панель** — исправлено: теперь можно **отвязать все привязанные устройства** у пользователя (пустой список устройств больше не игнорируется).
- **Dev** — симуляция размера окна в браузере (панель слева внизу) для подбора размеров.

### Changelog (2026-06-13, ч.3)

- **Окно** — размер изменён на **650×730** (`main_windows.go`, `main_linux.go`, dev-превью).
- **Сервер: пинг в играх (Steam Datagram Relay)** — исправлено отсутствие пинга/«Задержка: ОШИБКА» в Dota и др. играх.
  - **Причина:** ответы релеев Steam приходят пакетами ~1328 байт с флагом **DF**, которые не влезают в MTU туннеля (1280). Ядро в `ip_forward()` отбрасывает такие DF-пакеты **до** цепочки `FORWARD` и шлёт ICMP «fragmentation needed» — а он зарезан в ufw. В итоге UDP-ответы релеев не доходят до клиента.
  - **Фикс (server, `wdtt-xray-rules.sh`):** правило nft в `PREROUTING` (после reverse-NAT) сбрасывает DF на крупных пакетах, идущих в подсеть клиентов (`iifname WAN, daddr 10.66.66.0/24, length>MTU, frag DF → frag-off=0`). Ядро фрагментирует их под MTU туннеля, клиент собирает обратно. MTU туннеля при этом не меняется.
