...
| Путь | Содержание |
|------|------------|
| `docs/vk_apk_hosts.txt` | 122 хоста из dex (в репозитории) |
| `docs/VK_APK_AUDIT.md` | этот документ |
| APK `com.vkontakte.android` v8.183 | исходник разбора (локально) |

Полный apktool/smali-дамп (~210k smali) хранится на машине сборки; для WDTT достаточно dex-strings + нативных `.so`.
