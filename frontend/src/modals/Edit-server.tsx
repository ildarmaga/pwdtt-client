...
import { isPanelSubUrl, syncServerFromSubscription } from '../lib/utils/wdttLink';
...
export default function EditServer({ server, onClose, onSave, onDelete }: Props) {
  ...
  const [wbRoom, setWbRoom] = useState(server.wbRoom ?? '');
  const [hashes, setHashes] = useState<[string,string,string,string]>(
    server.hashes ?? ['', '', '', '']
  );
  const [hashOpen, setHashOpen] = useState(false);

  const globalOn = isVk && settingsStore.get().useGlobalHashes;
  ...
  useEffect(() => {
    if (!server.subUrl) return;
    if (!isVk && server.wbRoom) return;
    let cancelled = false;
    setSyncingSub(true);
    void syncServerFromSubscription(server).then(async (updated) => {
      if (cancelled) return;
      if (updated.wbRoom && updated.wbRoom !== server.wbRoom) {
        setWbRoom(updated.wbRoom);
      }
      if (updated.hashes && updated.hashes.some((h, i) => h !== (server.hashes?.[i] ?? ''))) {
        setHashes(updated.hashes ?? hashes);
      }
      if (updated.wbRoom || (updated.hashes?.some(h => h.trim()))) {
        serverStore.update(updated);
        await saveServerProfile(updated).catch(() => {});
      }
    }).finally(() => {
      if (!cancelled) setSyncingSub(false);
    });
    return () => { cancelled = true; };
  }, [server.id, server.subUrl]);
  ...
  const handleSave = async () => {
    ...
    const updated: Server = {
      ...
      wbRoom: wbRoom.trim() || undefined,
    };

    if (isVk) {
      updated.host = `${serverIp.trim()}:${serverPort.trim() || '56000'}`;
  ...
  return (
    <>
      <style>{`
        ...
        .es-hint-wb { font-size: 11px; color: var(--text-3); margin: 4px 0 10px; line-height: 1.4; padding: 10px 12px; background: var(--input-bg); border-radius: 10px; border: 1px solid var(--border); }
      ...
      <div className="es-overlay" onClick={onClose}>
        <div className="es-modal" onClick={e => e.stopPropagation()}>
          <div className="es-header">
            ...
            <span className="es-title">{isVk ? 'Профиль VK' : 'Профиль WB Stream'}</span>
            <span
              className="es-protocol-badge"
          ...
          {locked && (
            <p className="es-hint-lock">
              {isVk
                ? 'Сервер из подписки — название, адрес и URL нельзя изменить.'
                : 'Профиль из подписки — имя и room нельзя изменить.'}
            </p>
          )}
          ...
          {isVk ? (
            ...
            <input
              className={`es-input${locked ? ' es-input--locked' : ''}`}
              placeholder="WB Room / join link"
              value={wbRoom}
              readOnly={locked}
              onChange={e => { if (!locked) setWbRoom(e.target.value); }}
            />
          )}
          ...
          {isVk ? (
            <>
              ...
              >
                ...
                VK Хеши профиля ({filledHashes}/4)
              ...
              {globalOn && (
                <div className="es-hint" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--text-3)', marginBottom: 8 }}>
                  ...
                  Отключите «Глобальные хеши» в Настройках VK
                </div>
              )}
            </>
          ) : (
          ...
          <div className="es-btn-row">
            <button className="es-btn es-btn--save" onClick={handleSave} disabled={!name.trim() || (isVk && !serverIp.trim())}>Сохранить</button>
