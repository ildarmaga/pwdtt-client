...
type Config struct {
	...
	// tunnel (no SOCKS loopback). Nil keeps the legacy SOCKS5 engine path.
	Dialer TunnelDialer
...
func (t *Tunnel) Start() error {
	...
	for _, cidr := range lanBypassCIDRs {
		if out, err := runCmd("ip", "route", "add", cidr, "via", gw, "dev", iface); err != nil {
			t.log("[desktoptun] add LAN bypass %s failed: %v (%s)", cidr, err, out)
		} else {
...
func (t *Tunnel) Stop() {
	...
	for _, cidr := range lanBypassCIDRs {
		if _, err := runCmd("ip", "route", "del", cidr); err != nil {
...
func (t *Tunnel) RemoveBypassIP(ip net.IP) error {
	...
	iface := t.origIface
	t.mu.Unlock()
	if _, err := runCmd("ip", "route", "del", addr+"/32", "via", gw, "dev", iface); err != nil {
