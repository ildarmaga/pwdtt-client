...
type Config struct {
	...
	SocksUser   string
	SocksPass   string
	// Dialer is accepted for API parity with linux/windows; the darwin
	// path always uses the tun2socks SOCKS5 engine.
	Dialer TunnelDialer
...
func (t *Tunnel) RemoveBypassIP(ip net.IP) error {
	...
	gw := t.origGateway
	t.mu.Unlock()
	if _, err := runCmd("route", "-n", "delete", "-host", addr, gw); err != nil {
