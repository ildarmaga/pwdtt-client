...
type RelayBridge struct {
	...
	retiredConns sync.Map // connID -> time.Time (linger window after local close)
...
// SetMinVP8Pacing sets a floor for creator outbound VP8 pacing when the joiner
// negotiates slower values (joiner config only affects the creator's tunnel).
func (rb *RelayBridge) SetMinVP8Pacing(fps, batch int) {
	if fps > 0 {
		rb.minVP8FPS = fps
	}
	if batch > 0 {
		rb.minVP8Batch = batch
	}
}

// SetMaxVP8Pacing caps creator outbound VP8 pacing to avoid SFU congestion stalls.
func (rb *RelayBridge) SetMaxVP8Pacing(fps, batch int) {
	if fps > 0 {
		rb.maxVP8FPS = fps
	}
	if batch > 0 {
		rb.maxVP8Batch = batch
	}
}
...
func (rb *RelayBridge) SwapTunnel(newTunnel DataTunnel) {
	rb.SwapTunnelKeepConns(newTunnel, true)
}

// SwapTunnelKeepConns rebinds the data tunnel. When resetConns is false, live
// upstream/SOCKS TCP sessions are preserved (used on creator reconnect).
func (rb *RelayBridge) SwapTunnelKeepConns(newTunnel DataTunnel, resetConns bool) {
	rb.tunnelMu.Lock()
	rb.tunnel = newTunnel
	rb.tunnelMu.Unlock()
	...
	if resetConns {
		...
		rb.logFn("relay: tunnel swapped, keeping %d tcp + %d udp sessions", rb.countTCP(), rb.countUDP())
	}
}

func (rb *RelayBridge) countTCP() (n int) {
	rb.conns.Range(func(_, _ any) bool { n++; return true })
	return n
}

func (rb *RelayBridge) countUDP() (n int) {
	rb.udpClients.Range(func(_, _ any) bool { n++; return true })
	return n
}
...
func (rb *RelayBridge) closeAll() {
	...
	rb.retiredConns.Range(func(key, _ any) bool {
		rb.retiredConns.Delete(key)
		return true
	})
	rb.logFn("relay: closeAll mode=%s tcp=%d udp=%d ids=%v nextID=%d", rb.mode, len(ids), udpCount, ids, rb.nextID.Load())
}

const creatorConnLinger = 5 * time.Second

func (rb *RelayBridge) isRetiredConn(connID uint32) bool {
	v, ok := rb.retiredConns.Load(connID)
	if !ok {
		return false
	}
	until, ok := v.(time.Time)
	if !ok || time.Now().After(until) {
		rb.retiredConns.Delete(connID)
...
func (rb *RelayBridge) retireCreatorConn(connID uint32) {
	rb.retiredConns.Store(connID, time.Now().Add(creatorConnLinger))
	rb.nackedConns.Delete(connID)
}

func (rb *RelayBridge) finishCreatorTCP(connID uint32, conn net.Conn, reads int, totalRead int64, err error) {
	if v, loaded := rb.conns.LoadAndDelete(connID); !loaded {
		return
	} else if c, ok := v.(net.Conn); ok {
		conn = c
	}
	_ = conn.Close()
	rb.retireCreatorConn(connID)
	rb.send(connID, MsgClose, nil)
	if err != nil && err != io.EOF && !common.IsBenignConnError(err) {
		rb.logFn("relay: conn %d read error: %s (read %d times, %dB)", connID, common.MaskError(err), reads, totalRead)
	} else if reads == 0 && err == io.EOF {
		rb.logFn("relay: conn %d EOF with no data read", connID)
	}
}

func (rb *RelayBridge) creatorConnGone(connID uint32, msgType byte, _ int) bool {
	if rb.isRetiredConn(connID) {
		return true
	}
	switch msgType {
	case MsgClose:
		...
		return true
	case MsgData:
		if _, already := rb.nackedConns.LoadOrStore(connID, struct{}{}); !already {
			rb.send(connID, MsgClose, nil)
		}
		return true
	default:
...
func (rb *RelayBridge) handleTunnelData(data []byte) {
	DecodeFrames(data, func(connID uint32, msgType byte, payload []byte) {
		if connID == ControlConnID && msgType == MsgConfig {
			...
			if rb.mode == "creator" {
				effFPS, effBatch := fps, batch
				if rb.minVP8FPS > 0 && effFPS < rb.minVP8FPS {
					effFPS = rb.minVP8FPS
				}
				if rb.minVP8Batch > 0 && effBatch < rb.minVP8Batch {
					effBatch = rb.minVP8Batch
				}
				if rb.maxVP8FPS > 0 && effFPS > rb.maxVP8FPS {
					effFPS = rb.maxVP8FPS
				}
				if rb.maxVP8Batch > 0 && effBatch > rb.maxVP8Batch {
					effBatch = rb.maxVP8Batch
				}
				rb.logFn("relay: peer requested vp8 pacing fps=%d batch=%d trackCount=%d (effective fps=%d batch=%d)", fps, batch, trackCount, effFPS, effBatch)
				rb.currentTunnel().Reconfigure(effFPS, effBatch)
...
func (rb *RelayBridge) handleCreatorMessage(connID uint32, msgType byte, payload []byte) {
	...
	case MsgData:
		val, ok := rb.conns.Load(connID)
		if !ok {
			if rb.creatorConnGone(connID, msgType, len(payload)) {
				return
			}
...
func (rb *RelayBridge) connectTCP(connID uint32, addr string) {
	...
	var err error
	if rb.upstream != nil {
		conn, err = rb.upstream.DialTCP(addr, 30*time.Second)
	} else {
	...
	common.EnableTCPKeepAlive(conn, 30*time.Second)
	...
	for {
		...
		if err != nil {
			rb.finishCreatorTCP(connID, conn, reads, totalRead, err)
			return
		}
	}
}
...
func (rb *RelayBridge) handleSOCKS(conn net.Conn) {
	...
	if rb.mode == "joiner" && common.IsNonRoutableHost(host) {
		if ip := net.ParseIP(hostOnly); ip != nil && ip[0] == 198 {
			rb.logFn("relay: SOCKS reject fake-dns IP %s (v2ray: route Telegram by domain to SOCKS, disable fake-ip on this outbound)", common.MaskAddr(host))
			conn.Write(common.ConnFail)
			conn.Close()
			return
		}
		...
		go func() {
			...
			go func() {
				_, _ = sessionstats.Copy(target, conn, false)
				close(done)
			}()
			_, _ = sessionstats.Copy(conn, target, true)
			<-done
		}()
		return
	}
	...
	go func() {
		...
		for {
			...
			if rerr != nil {
				...
				rb.conns.Delete(id)
				// Benign closes (client/app closed the socket, RST, EOF) are the
				// normal end of a SOCKS connection — don't spam them as errors.
				if !common.IsBenignConnError(rerr) {
