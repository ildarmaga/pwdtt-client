...
func getTokenChain(ctx context.Context, link string, streamID int, creds VKCredentials, jar tlsclient.CookieJar, captchaResultChan chan string, getCaptchaMode func() string, emitCaptchaRequest func(mode, redirectURI, sessionToken string)) (string, string, []string, error) {
	...
	client, err := newVKHTTPClient(jar)
	if err != nil {
	...
	data = fmt.Sprintf("vk_join_link=%s&fields=photo_200&access_token=%s", vkCallJoinURL(link), token1)
	...
	data = fmt.Sprintf("vk_join_link=%s&name=%s&access_token=%s", vkCallJoinURL(link), escapedName, token1)
		...
		if errObj, hasErr := resp["error"].(map[string]interface{}); hasErr {
			...
			if captchaErr != nil && captchaErr.RedirectURI != "" && captchaErr.SessionToken != "" {
				...
				data = fmt.Sprintf("vk_join_link=%s&name=%s&captcha_key=&captcha_sid=%s&is_sound_captcha=0&success_token=%s&captcha_ts=%s&captcha_attempt=%s&access_token=%s",
					vkCallJoinURL(link), escapedName, captchaErr.CaptchaSid, neturl.QueryEscape(successToken), captchaErr.CaptchaTs, captchaAttempt, token1)
				continue
			}
...
// vkOKJoinLink extracts OK CDN join token from VK API response (ok_join_link).
func vkOKJoinLink(fallback string, resp map[string]interface{}) string {
	if resp == nil {
		return fallback
	}
	if m, ok := resp["response"].(map[string]interface{}); ok {
		for _, key := range []string{"ok_join_link", "join_link"} {
			if s, ok := m[key].(string); ok && strings.TrimSpace(s) != "" {
				return vkExtractJoinToken(s, fallback)
			}
		}
	}
	return fallback
...
func vkExtractJoinToken(raw, fallback string) string {
...
func vkOKCDNError(resp map[string]interface{}) string {
	if resp == nil {
		return ""
	}
...
func setupGlobalResolver() {
	// Do not override net.DefaultResolver with public DNS over UDP — on
	// whitelisted networks 8.8.8.8/77.88.8.8 UDP/53 is blocked while the
	// router DNS (192.168.8.1) works. iOS app pre-resolves VK IPs instead;
