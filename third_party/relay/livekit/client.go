...
type Client struct {
	...
	OnReady              func()
	OnJoin               func(JoinResponse)
	OnTrack              func(*webrtc.TrackRemote, *webrtc.RTPReceiver)
	OnDataChannel        func(*webrtc.DataChannel)
	OnPubConnected       func()
	OnSubICEConnected    func()
	OnSubICEDisconnected func()
	OnSubOfferApplied    func()
...
func (c *Client) buildPeerConnections() error {
	...
	pubPC.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateConnected {
			// pub carries our outbound VP8 (the upload path that wedges). Log the
			// transport ICE actually chose (UDP srflx vs TCP/UDP TURN relay) so a
			// field wedge can be correlated with the carrier type + its RTT.
			go c.logSelectedPair(pubPC, "pub")
			if c.OnPubConnected != nil {
				c.OnPubConnected()
			}
		}
	})
	...
	subPC.OnICEConnectionStateChange(func(state webrtc.ICEConnectionState) {
		...
		case webrtc.ICEConnectionStateConnected:
			go c.logSelectedPair(subPC, "sub")
			if c.OnSubICEConnected != nil {
				c.OnSubICEConnected()
			}
		case webrtc.ICEConnectionStateFailed,
			webrtc.ICEConnectionStateClosed:
			if c.OnSubICEDisconnected != nil {
				c.OnSubICEDisconnected()
			}
		}
	})
...
// logSelectedPair reports which ICE candidate pair actually carries this PC's
// media once it is nominated. It is pure observability (no config change): the
// pub PC's pair is the outbound/upload transport, and knowing whether it is a
// UDP srflx path or a TCP/UDP TURN relay — plus its live RTT — is what lets us
// tell a carrier-transport wedge from an application one. GetSelectedCandidatePair
// can be briefly empty right after connect, so poll a few times.
func (c *Client) logSelectedPair(pc *webrtc.PeerConnection, label string) {
	for i := 0; i < 10; i++ {
		sctp := pc.SCTP()
		if sctp != nil {
			if dtls := sctp.Transport(); dtls != nil {
				if it := dtls.ICETransport(); it != nil {
					if pair, err := it.GetSelectedCandidatePair(); err == nil && pair != nil && pair.Local != nil && pair.Remote != nil {
						c.logFn("[lk] %s selected ICE pair: local=%s/%s remote=%s/%s",
							label, pair.Local.Protocol, pair.Local.Typ,
							pair.Remote.Protocol, pair.Remote.Typ)
						return
					}
				}
			}
	...
	c.logFn("[lk] %s selected ICE pair: unavailable", label)
}
...
func (c *Client) PingLoop() {
	...
	for range t.C {
		...
		if err := c.SendPing(); err != nil {
			...
			c.Close() // unblock ReadLoop so session can reconnect
			return
		}
		sentN++
