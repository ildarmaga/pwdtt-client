...
func (j *WBStreamHeadlessJoiner) RunWithParams(jsonParams string) {
	...
	if err := j.runOnce(httpClient, params.RoomID, params.DisplayName, params.TunnelMode, settingEngine, params.VP8FPS, params.VP8Batch, params.DualTrack, &attempt); err != nil {
...
func (j *WBStreamHeadlessJoiner) runOnce(httpClient *http.Client, roomID, displayName, tunnelMode string, settingEngine *webrtc.SettingEngine, vp8FPS, vp8Batch int, dualTrack bool, attempt *atomic.Int32) error {
	// Fresh obfuscator per session so creator sees a new peer epoch after reconnect
	// and resets KCP to match joiner's SwapTunnel.
	obf, err := tunnel.NewTunnelObfuscator(tunnel.DeriveSecretFromJoinLink(roomID))
	if err != nil {
		return fmt.Errorf("obfuscator: %w", err)
	}
	j.logFn("wbstream-joiner: obf localEpoch=0x%08x", obf.LocalEpoch())
