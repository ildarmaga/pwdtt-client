...
### Feature — остаток срока обычного и промо-устройства
- Статистика подписки передаёт `device_id` панели и читает `expire_kind` / `expire_state` / `remaining_seconds`.
- Карточка Connect показывает «Бессрочно», общий срок обычного пользователя или срок текущего промо-устройства, включая состояние до первой привязки.
- `DENIED:device_expired` отображается как «Срок устройства истёк».
...
- Добавлены безопасные лимиты потоков и проверка входящих кадров без изменения WB1 v3 wire-протокола.
- Исправлены вложенные SOCKS UDP-адреса и корректный TCP half-close drain.
- Reconnect клиента теперь надёжно повторяется с ограниченным backoff после ошибок подключения.

## [0.3.323] — 2026-08-19

### Fix — домены без зависимости от DNS-маршрута
- RAW/WireGuard теперь извлекают домен из TLS SNI и HTTP Host исходящего соединения, поэтому домены видны даже когда системный DNS идёт мимо туннеля или используется защищённый DNS.
- Содержимое соединений не сохраняется; в памяти остаётся только найденное имя и счётчики байтов.

## [0.3.322] — 2026-08-19

### Feature — трафик во всех режимах с доменами
- Вкладка переименована в «ТРАФИК» и теперь считает RAW, WireGuard и WB/SOCKS.
- WB показывает домены прямо из SOCKS-запросов; RAW/WireGuard сопоставляют IP с доменами из DNS-ответов внутри туннеля, без внешних запросов.
- Если приложение использует защищённый DNS и домен невозможно узнать локально, сохраняется IP как честный запасной вариант.

## [0.3.321] — 2026-08-19

### Feature — локальная разбивка RAW-трафика
- На экране логов появилась вкладка «КУДА УШЛО» с топом удалённых IP:портов и отдельными объёмами загрузки/отправки.
- Учёт ведётся только в памяти текущей RAW-сессии, содержимое пакетов не сохраняется и внешние DNS-запросы не выполняются.

## [0.3.320] — 2026-08-18

### Fix — WB адаптируется к перегрузке и не рвёт весь mux
- Стартовый VP8-поток снижен до безопасных 16 Мбит/с и автоматически меняется в диапазоне 4–24 Мбит/с по задержке LiveKit WriteSample.
- Временный WB-затор больше не уничтожает весь mux через 10 секунд; полная тишина подтверждается 30 секунд, а клиент пересоздаёт реально умершую сессию после трёх ping с подробной диагностикой.
- Резервные ACK/Ping через LiveKit data ограничены очередью 64 вместо неограниченных goroutine.
...
- 18 живых воркеров с ~10 КиБ/3с больше не считаются data-path. Раньше verify «успевал», лог «VK пока напрямую, ждём data-path» шёл каждые 3 с, stall-immune обновлялся и reconnect не стартовал.
- Keepalive 18 воркеров (до 24 КиБ/тик) не двигает часы трафика. Нет реального трафика за 45 с после soft → повторный soft, затем полный reconnect.
- VK API по-прежнему напрямую (не через туннель).

## [0.3.312] — 2026-08-16

### Fix — WB SOCKS переподключается когда интернет умер
- WBT 0 и простой трафика больше не игнорируются. Раньше вотчдог пропускал «нет живого RTT» и сессия висела с 0 B/s без reconnect.
- Пока байты ещё идут (спидтест), мёртвый ping по-прежнему не рвёт сессию.

## [0.3.311] — 2026-08-15

### Fix — WB SOCKS не рвётся на спидтесте
- Один застрявший ARQ-сегмент больше не закрывает весь mux. Раньше через ~10 с WBT становился 0 и нужен был reconnect.
- Mux закрывается только если ACK совсем не двигаются.

## [0.3.310] — 2026-08-15

### Fix — RAW сайты + любые игры
- UDP :443 (QUIC/HTTP/3) целиком пачками по 12, включая мелкие ACK. Иначе Chrome рвёт сайты.
- Остальной UDP/ICMP sticky на одном TURN — любая игра, не только Steam.
...
- Кадр `WB1\x03`, MaxPayload 1024, ARQ на SID-пару (окно 32, SACK). SOCKS/TLS больше не рвётся после 0.5–2 КБ из‑за VP8-фрагментации без сборки.
- Нужна панель ≥1.5.32 (matched pair). v1/v2 отбрасываются явно.

## [0.3.303] — 2026-08-14

### Fix — WB1 v2, несколько пользователей в одной комнате
- Кадр `WB1\x02` с dest/src SID. SOCKS не смешивается, даже если SFU ставит SenderIdentity `e8214196`.
- Mux только VP8; data-topic — маяк creator. Нужна панель ≥1.5.31 (взаимно, v1 не смешивается).
...
- При подключении WB Stream в LiveKit DisplayName уходит `device_id` профиля (раньше всегда `"WDTT"`).
- Панель ≥1.5.7 матчит этот id и красит тег устройства зелёным.
...
- После потери всех воркеров сначала soft (сохраняем `wg-turn`).
- Если воркеры поднялись, а трафик за ~45 с не пошёл → **полный** reconnect (снос TUN/маршрутов).
- Повторный stall / второй workers=0 тоже эскалируется в full (лимит soft=1).
- Чинит zombie после рестарта сервера: soft оставлял старый WG peer, UI «жив», данные нет.

## [0.3.277] — 2026-08-10

### Fix — TS build (релиз 0.3.276)
- `wdttLink.ts`: не затенять параметр `raw` полем JSON `raw` (tsc TS2448/TS6133).

## [0.3.276] — 2026-08-10

### RAW порт из ссылки/профиля
- Парсит `raw` из `wdtt://` / подписки; пишет `raw_port` в профиль.
- Direct WRAP peer: явный порт или по умолчанию DTLS+3.
- Нужен server ≥ 1.4.75 (порт в панели); со старым 1.4.74 работает DTLS+3.
- CI: починен `NewDispatcher` в sticky-тестах (0.3.275 не собрался).
...
- **WG TCP/UDP** как в 0.3.267: настройки транспорта не форсятся.
- Убран авто-UDP/авто-TCP при смене режима.
- RAW sticky по умолчанию (как было); multipath только `RAW_MULTIPATH=1`.
- Правки bench/reorder на WG не влияют.
...
Новые TCP-потоки садятся на воркер с мин. очередью+RTT (не слепой hash%N).
`cmd/wdtt-bench` — замер скорости на сервере через netns (без скачивания exe).
Эксперимент RAW multipath (`RAW_MULTIPATH=1`) на стенде хуже sticky — по умолчанию выкл.
...
Режим RTP-обфускации: **Аудио** (OPUS PT 111) / **Видео** (VP8-like PT 96).

- Настройки → VK · TURN → «Маскировка» (сегмент Аудио/Видео)
- Клиент wrap: video → PT 96, padding до 60 байт
- Unwrap принимает оба PT; сервер зеркалит PT клиента на обратном пути
- Нужен wdtt-server с поддержкой PT 96 (этот же релиз инфраструктуры)

## [0.3.250] — 2026-07-21

### Fix — плашка Cookies/Гость сразу после тумблера
- Убрана подсказка «Вход через VK — на главном…» из Настроек
- Переключение тумблера / save/clear cookies сразу обновляет плашку на главной

## [0.3.249] — 2026-07-21

### Fix — вход VK слева, режим только индикатор
- Сайдбар: кнопка VK (вход)
- В панели сессии: плашка **Cookies / Гость** без клика (как SOCKS-индикатор)

## [0.3.248] — 2026-07-21

### Fix — режим VK как плашка SOCKS
Убран текстовый блок под VK/WB. Режим **Cookies / Гость** — компактная
плашка в панели сессии (как SOCKS у WB); клик открывает вход VK.

## [0.3.247] — 2026-07-21

### Change — вход VK на главном экране
- (переделано в 0.3.248 на плашку как SOCKS)
- Кнопку входа убрали из Настроек (остались тумблер, статус, ручной ввод)

## [0.3.246] — 2026-07-21

### Fix — настройки VK cookies: статус и текст после входа
Файл `cookies-vk.json` уже писался, но UI не читал его в textarea и не
показывал статус, когда cookies OK (hint был пустой).

- `GetVKCookiesRaw` → поле cookies заполняется из файла
- Статус всегда: действительны / устарели / не заданы
- После «Войти через VK» refresh подтягивает текст и статус
...
- Корень: `go-webview2` на любой COM-ошибке WebView2 вызывал `os.Exit(1)` (в т.ч. из `Resize`/`Navigate` после краша render). Воркер входа умирал молча.
- Форк в `third_party/go-webview2`: runtime-ошибки без `os.Exit`; жёсткий exit только на bootstrap.

### UI — отмена скачивания обновления
- Кнопка «Отменить» во время скачивания; после отмены можно снова нажать «Скачать» / «Установить».

## [0.3.232] — 2026-07-21

### Fix — обновление скачивается при включённом VPN
- Раньше апдейтер всегда уходил мимо туннеля (DirectBypass + LAN bind). В РФ GitHub с «голого» ISP часто недоступен → зависание на «Скачивание…», а в браузере через VPN качается нормально.
- Пока WDTT VPN активен — качаем **через туннель** (как браузер). Bypass LAN — только когда VPN выключен.

## [0.3.231] — 2026-07-21

### Fix — вход VK: окно QR больше не закрывается через 1–2 сек
- WebView2 `ProcessFailedCallback`: только смерть browser-процесса фатальна. Крэш render/GPU/frame (тяжёлый QR-canvas VK = пустой белый QR) теперь не убивает окно, а перезагружает страницу.
- React `VKAuth`: cleanup больше не вызывает `StopVKLogin` для нативного окна. Убрана нестабильная зависимость эффекта `[onClose, onDone]` (новые inline-функции на каждый ре-рендер `Settings` → teardown → kill worker). Native-воркер сам управляет своим жизненным циклом; убивается только явным закрытием (крестик).
