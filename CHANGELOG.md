...
### Fix — сброс залипшего UDP + WG на TCP
- При обновлении сбрасывает TURN обратно на **TCP** (0.3.268 форсил UDP → у тебя 0 байт на WG·UDP).
- Выбор **WG** ставит TCP. Не форсим UDP при RAW.
- Ставь **v0.3.270**, в настройках проверь **TURN = TCP**, режим **WG**.
