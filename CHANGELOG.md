...
### Исправлено
- **Быстрый старт трафика**: wg_config может запросить любой воркер группы #1 (не только первый); группа #2 стартует после GETCONF или через 500 ms; stagger воркеров 450 ms вместо 1.2 s.
